...
[data-only module](/language/modules/develop/composition#data-only-modules)
...
* (v0.12+) `outputs` - An object containing every root-level
  [output](/language/values/outputs) in the remote state.
* (<= v0.11) `<OUTPUT NAME>` - Each root-level [output](/language/values/outputs)
